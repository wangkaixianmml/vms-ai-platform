...
function deepMerge(source, ...objects) {
  ...
  for (const obj of objects) {
    if (!utils.isPlainObject(obj)) continue;
    const target = utils.compact(obj);
    for (const key in target) {
      if (!Object.prototype.hasOwnProperty.call(target, key)) continue;
      if (key === "__proto__" || key === "constructor" || key === "prototype") continue;
      const sourceVal = source[key];
      const targetVal = obj[key];
      if (utils.isPlainObject(targetVal)) {
        source[key] = utils.isPlainObject(sourceVal) ? deepMerge(sourceVal, targetVal) : { ...targetVal };
      } else {
...
function toEvent(event) {
  const obj = utils.isString(event) ? { type: event } : event;
  return obj;
}
function toArray(value) {
  if (!value) return [];
  return utils.isArray(value) ? value.slice() : [value];
}
function isGuardHelper(value) {
  return utils.isObject(value) && value.predicate != null;
}
...
function determineGuardFn(guard, guardMap) {
  ...
  return (context, event, meta) => {
    if (utils.isString(guard)) {
      ...
      return utils.isFunction(value) ? value(context, event, meta) : value;
    }
...
function createProxy(config) {
  const computedContext = config.computed ?? utils.cast({});
  const initialContext = config.context ?? utils.cast({});
  const initialTags = config.initial ? config.states?.[config.initial]?.tags : [];
  const state = store.proxy({
    value: config.initial ?? "",
    previousValue: "",
    event: utils.cast({}),
    previousEvent: utils.cast({}),
    ...
    can(event) {
      return utils.cast(this).nextEvents.includes(event);
    },
    get nextEvents() {
  ...
  return (context, event) => {
    ...
    if (utils.isString(delay)) {
      ...
      if (delaysMap) {
        ...
        return utils.isFunction(valueOrFn) ? valueOrFn(context, event) : valueOrFn;
      }
    }
  };
}
function toTarget(target) {
  return utils.isString(target) ? { target } : target;
}
...
var Machine = class {
  ...
  constructor(config, options) {
    ...
    __publicField(this, "removeStateListener", utils.noop);
    ...
    __publicField(this, "start", (init) => {
      ...
      this.removeStateListener = store.subscribe(
        this.state,
        () => {
      ...
      const target = utils.isObject(init) ? init.value : init;
      const context = utils.isObject(init) ? init.context : void 0;
      if (context) {
    ...
    __publicField(this, "setupContextWatchers", () => {
      ...
      if (!watch) return;
      let prev = store.snapshot(this.state.context);
      const cleanup = store.subscribe(this.state.context, () => {
        const next = store.snapshot(this.state.context);
    ...
    __publicField(this, "sendChild", (evt, to) => {
      ...
      const id = utils.runIfFn(to, this.contextSnapshot);
      const child = this.children.get(id);
      if (!child) {
        utils.invariant(`[@zag-js/core] Cannot send '${event.type}' event to unknown child`);
      }
      child.send(event);
    });
    /**
    ...
    __publicField(this, "stopChild", (id) => {
      if (!this.children.has(id)) {
        utils.invariant(`[@zag-js/core > stop-child] Cannot stop unknown child ${id}`);
      }
    ...
    __publicField(this, "spawn", (src, id) => {
      const actor = utils.runIfFn(src);
      if (id) actor.id = id;
      actor.type = "machine.actor" /* Actor */;
      actor.setParent(this);
      this.children.set(actor.id, utils.cast(actor));
      actor.onDone(() => {
        this.removeChild(actor.id);
      }).start();
      return utils.cast(store.ref(actor));
    });
    ...
    __publicField(this, "setOptions", (options) => {
      const opts = utils.compact(options);
    ...
    __publicField(this, "getDelayedEventActions", (state) => {
      ...
      const entries = [];
      const exits = [];
      if (utils.isArray(stateNode.after)) {
        const transition = this.determineTransition(stateNode.after, event);
        if (!transition) return;
        if (!utils.hasProp(transition, "delay")) {
      ...
      if (utils.isObject(stateNode.after)) {
    ...
    __publicField(this, "executeActions", (actions, event) => {
      ...
      for (const action of toArray(pickedActions)) {
        const fn = utils.isString(action) ? this.actionMap?.[action] : action;
        utils.warn(
          utils.isString(action) && !fn,
    ...
    __publicField(this, "executeActivities", (event, activities, state) => {
      for (const activity of activities) {
        const fn = utils.isString(activity) ? this.activityMap?.[activity] : activity;
        if (!fn) {
          utils.warn(`[@zag-js/core > execute-activity] No implementation found for activity: \`${activity}\``);
          continue;
        }
        const cleanup = fn(this.state.context, event, this.meta);
        if (cleanup) {
          const key = utils.isString(activity) ? activity : activity.name || utils.uuid();
    ...
    __publicField(this, "setEvent", (event) => {
      ...
      this.state.event = store.ref(toEvent(event));
    });
    ...
    __publicField(this, "sendParent", (evt) => {
      if (!this.parent) {
        utils.invariant("[@zag-js/core > send-parent] Cannot send event to an unknown parent");
      }
      const event = toEvent(evt);
      this.parent?.send(event);
    });
    __publicField(this, "log", (...args) => {
      if (utils.isDev() && this.options.debug) {
        console.log(...args);
      }
    });
    /**
    ...
    __publicField(this, "transition", (state, evt) => {
      const stateNode = utils.isString(state) ? this.getStateNode(state) : state?.stateNode;
      const event = toEvent(evt);
    ...
    this.initialContext = store.snapshot(this.state.context);
  }
  // immutable state value
  get stateSnapshot() {
    return utils.cast(store.snapshot(this.state));
  }
  getState() {
...
Object.defineProperty(exports, "proxy", {
  enumerable: true,
  get: function () { return store.proxy; }
});
Object.defineProperty(exports, "ref", {
  enumerable: true,
  get: function () { return store.ref; }
});
Object.defineProperty(exports, "snapshot", {
  enumerable: true,
  get: function () { return store.snapshot; }
});
Object.defineProperty(exports, "subscribe", {
  enumerable: true,
  get: function () { return store.subscribe; }
});
